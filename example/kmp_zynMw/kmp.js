...
async function searchSongs(keyword, page = 0, append = false) {
  ...
  loading = true;
  try {
    const apiUrl = `https://search.kuwo.cn/r.s?all=${encodeURIComponent(keyword)}&pn=${page}&rn=20&vipver=100&ft=music&encoding=utf8&rformat=json&vermerge=1&mobi=1&itemset=web_2013&client=kt`;
    ...
    // 判断是否还有更多
    hasMore = (data.PN || page) * (data.RN || 20) < (data.TOTAL || 0);
    currentPage = page;
  } catch (e) {
    if (!append)
      resultsDiv.innerHTML =
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  audioBar.classList.add("show-hide-btn");
  if (audioHideBtn) audioHideBtn.style.display = "flex";
  audioBar.style.boxShadow = "none";
  audioBar.style.zIndex = 9;
  document.body.style.overflow = "hidden";
});
