...
contextBridge.exposeInMainWorld('pluginAPI', {
  ...
  register: (pluginId, functions) => { ipcRenderer.send('plugin:register', pluginId, functions); },
  ...
  onInvoke: (handler) => {
    ipcRenderer.on('plugin:invoke', async (_e, payload) => {
      try { const result = await handler(payload.fn, payload.args); ipcRenderer.send('plugin:invoke:result', payload.id, { ok: true, result }); }
      catch (err) { ipcRenderer.send('plugin:invoke:result', payload.id, { ok: false, error: err?.message || String(err) }); }
    });
  },
  ...
  registerAutomationEvents: (pluginId, events) => { ipcRenderer.send('plugin:automation:register', pluginId, events); },
