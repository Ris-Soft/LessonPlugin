...
(async () => {
  ...
  canvas.on('path:created', (e) => {
    try {
      const p = e.path;
      if (!p) return;
      if (state.mode === 'draw') { p.erasable = true; setTimeout(() => { if (!history.restoring) { try { history.recordHistory(); } catch {} history.updateUndoRedoUI(); } }, 0); }
      else if (state.mode === 'erase' && fabric && fabric.EraserBrush) { p.selectable = false; p.evented = false; p.isEraser = true; setTimeout(() => { if (!history.restoring) { try { history.recordHistory(); } catch {} history.updateUndoRedoUI(); } }, 0); }
    } catch {}
  });
  ...
  canvas.on('mouse:up', () => { if ((state.mode === 'erase' || state.mode === 'draw') && !history.restoring) { try { history.recordHistory(); } catch {} } draggingErase = false; draggingPan = false; history.updateUndoRedoUI(); });
  canvas.on('mouse:wheel', (opt) => { const e = opt.e; let delta = e.deltaY; let zoom = canvas.getZoom(); zoom *= 0.999 ** delta; zoom = Math.max(0.2, Math.min(3, zoom)); const p = typeof canvas.getPointer === 'function' ? canvas.getPointer(e) : { x: e.offsetX, y: e.offsetY }; canvas.zoomToPoint(new fabric.Point(p.x, p.y), zoom); resizeEraseCursor(state.eraseSize); opt.e.preventDefault(); opt.e.stopPropagation(); });
